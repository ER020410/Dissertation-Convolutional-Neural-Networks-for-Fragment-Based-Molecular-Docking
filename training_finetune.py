# -*- coding: utf-8 -*-
"""training_finetune.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1n2rF1jRz0SXDxr4jguL8gecuersQH0IL
"""
...
def options(args: Optional[List[str]] = None):
    ...
    nargs="+",
    default=[],
    help="List of layer name prefixes to freeze (e.g., features.unit1 features.unit2)",
    )
    parser.add_argument(
        "-i",
        "--iterations",
...
def training(args):
    ...
    # 1. 构建模型（pose-only，不要 affinity）
    model = models_dict[(args.model, False, False)](train_loader.dims).to(device)
    ...
    for k, v in state_dict.items():
        if k.startswith("unit"):                      # 卷积层
            new_state_dict["features." + k] = v
        elif k.startswith("pose_output"):             # pose 输出层
            new_state_dict["pose." + k] = v
        elif "affinity_output" in k:                  # 跳过 affinity 层
            continue
        else:
            new_state_dict[k] = v
    ...
    # 冻结前几层
    freeze_prefixes = args.freeze_prefixes
